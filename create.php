...
switch ($post_data['insert_type']) {
	...
	case 'singer':
		...
		if(!$row){
			...
			// 不存在 -> 新增
			$sql = "INSERT INTO singers (name,type) VALUES ('{$post_data['name']}','{$singer_type}')";
			mysqli_query($con,$sql);
		}
		break;
	...
	case 'album':
		...
		if(!$row){
			...
			$sql = "INSERT INTO albums (name,cate_id,singer_info,song_info) VALUES ('{$post_data['name']}','{$post_data['cate_id']}','{$singer_info}','{$song_info}')";
			mysqli_query($con,$sql);
	...
	case 'playlist':
		// 判斷是否已存在
		$chk_sql = "SELECT * FROM playlists WHERE name='{$post_data['name']}' AND u_id={$post_data['u_id']}";
		$res = mysqli_query($con,$chk_sql);
		$row = mysqli_fetch_assoc($res);
		if(!$row){
			$song_info = implode(',', $post_data['song']);
			// 不存在 -> 新增
			$sql = "INSERT INTO playlists (u_id,name,song_info) VALUES ({$post_data['u_id']},'{$post_data['name']}','{$song_info}')";
			mysqli_query($con,$sql);
		}
		break;
	default:
